...
internal static class XmlDeserialiser
{
    ...
    internal static List<T> XmlListDeserialise<T>(string xml, string root, string obj, string? template_match = null, DataAcceptancePolicy policy = DataAcceptancePolicy.DeclineWholeFileOnInvalid) where T : class
    {
        XmlDocument doc = new();
        doc.LoadXml(xml);
        ...
        XmlSerializer serialiser = new(typeof(T), new XmlRootAttribute("LanguageType"));
        List<T> list = new();
        foreach (XmlNode node in nodes)
        {
            try
            {
                ...
                T? element = (T?)serialiser.Deserialize(reader);
